...
template <typename T>
using has_usart6_irq = decltype(T::USART6_IRQn);

template <typename T>
using has_usart7_irq = decltype(T::UART7_IRQn);

template <typename T>
using has_usart8_irq = decltype(T::UART8_IRQn);

template <typename T>
using has_usart9_irq = decltype(T::UART9_IRQn);

template <typename T>
using has_usart10_irq = decltype(T::UART10_IRQn);
...
template<typename T, int usart_number>
struct get_irq<T, usart_number, typename std::enable_if_t<
    std::experimental::is_detected_v<has_usart6_irq, T>
    && (6 == usart_number)
>>
{
    static constexpr T value = T::USART6_IRQn;
};

template<typename T, int usart_number>
struct get_irq<T, usart_number, typename std::enable_if_t<
    std::experimental::is_detected_v<has_usart7_irq, T>
    && (7 == usart_number)
>>
{
    static constexpr T value = T::UART7_IRQn;
};

template<typename T, int usart_number>
struct get_irq<T, usart_number, typename std::enable_if_t<
    std::experimental::is_detected_v<has_usart8_irq, T>
    && (8 == usart_number)
>>
{
    static constexpr T value = T::UART8_IRQn;
};

template<typename T, int usart_number>
struct get_irq<T, usart_number, typename std::enable_if_t<
    std::experimental::is_detected_v<has_usart9_irq, T>
    && (9 == usart_number)
>>
{
    static constexpr T value = T::UART9_IRQn;
};

template<typename T, int usart_number>
struct get_irq<T, usart_number, typename std::enable_if_t<
    std::experimental::is_detected_v<has_usart10_irq, T>
    && (10 == usart_number)
>>
{
    static constexpr T value = T::UART10_IRQn;
};
